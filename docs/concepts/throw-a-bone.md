...
* Create a spray when you throw the bone by adding ``||sprites:mySprite start spray effect||`` in the ``||controller:on A button pressed||`` event, and make the spray come from the bone