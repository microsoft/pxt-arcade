...
![Play Pickle Party](/static/tutorials/lyla/lyla.gif "You can play with up to 4 people!")
...
the choose the first background image called **diner**.
...
mp.onControllerEvent(ControllerEvent.Connected, function (player2) {
    pickle.setPlayersWith(characters, mp.getPlayerProperty(player2, mp.PlayerProperty.Number))
})
```
...
game.onUpdateInterval(500, function () {
    projectile = sprites.createProjectileFromSide(lyla_imgs, randint(-100, 100), randint(-100, 100))
})
```
...
<br/>
This block launches the ** projectile** of your choosing from the side of the screen with a **velodiner** that's randomly assigned each time the code runs.


```block
    projectile = sprites.createProjectileFromSide(lyla_imgs.pickle-chip, randint(-100, 100), randint(-100, 100))
...
~hint What is velodiner? 🕵🏻‍♀️

---

**VELOdiner** is the speed that something is moving in a certain direction.

In MakeCode, we have velodiner in two directions, **vx** and **vy**.

The field **vx** is for the velodiner going side to side.<br/>

_Velodiner in the x (horizontal) direction_
...
The field **vy** is for the velodiner going up and down.<br/>

_Velodiner in the y (vertical) direction_
...
<br/>
This **event** block triggers an action every time one of your Player sprites overlaps one of the projectile sprites. (Whenever a character overlaps the pickle.)
...
<br/>
This block adds one point to the score of the Player that overlapped the pickle.
...
Each character should get a point when they overlap pickle.
...
scene.setBackgroundImage(assets.image`dinerscape`)
pickle.setPlayersWith (characters, 1)
...
arcade-block-icons=github:jwunderl/arcade-block-icons/
```
