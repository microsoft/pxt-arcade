...
This was created for the [Motion Project](/courses/csintro/motion-and-events/project), but the process applies throughout the course. It is written from the perspective of a group, but the projects can also be completely as a "group of one."
...
![Initial Drawing](/static/courses/csintro/project/trampoline-drawing.png)
...
We looked back at the lessons to see if we could find any way to implement this, and noticed that the [Sprite Overlap 2](/courses/csintro/motion-and-events/overlap2.md) lesson showed us how to "bump" a sprite in the opposite direction when they overlapped with another sprite.
...
When we started working on making more sprites, we noticed that this seemed similar to the clouds from the [Create and On Create Sprites](/courses/csintro/motion-and-events/create-on-create-sprites) lesson, so we decided to use that for making the trampolines.
