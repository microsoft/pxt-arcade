...
Remember that ``||loops:for||`` loops are different than ``||loops:repeat||`` loops in that the for loop introduces a variable that is automatically incremented each time through the loop.
